...
class GraphRAG:
    ...
    chunk_overlap_token_size: int = 70
    ...
    def __post_init__(self):
        ...
        self.triples_vdb = (
            self.vector_db_storage_cls(
                ...
                global_config=asdict(self),
                embedding_func=self.embedding_func,
                meta_fields={"source_e_name", "target_e_name"},
            )
        )
    ...
    async def aquery(self, query: str, param: QueryParam = QueryParam()):
        print(f"query mode: {param.mode}")
        if param.mode == "my_query_context":
            context, text_units_context, path, chunk_list = await my_query_context(
                    query,
        ...
        if param.mode == "my_query":
            response, context, text_units_context, path, chunk_list = await my_query(
                    query,
                    self.chunk_entity_relation_graph,
                    self.entities_vdb,
                    self.triples_vdb,
            ...
            response, chunk_list = await local_query(
                query,
                self.chunk_entity_relation_graph,
                self.entities_vdb,
            ...
            response, chunk_list = await naive_query(
                query,
                self.chunks_vdb,
                self.text_chunks,
                param,
    ...
    async def ainsert(self, string_or_strings):
        ...
        try:
            ...
            if self.enable_naive_rag:
                logger.info("Insert chunks to vector db")
                await self.chunks_vdb.upsert(inserting_chunks)
            # insert chunks to graph
            logger.info("Insert chunks to graph")
            # inserting_chunks format: {chunk_id: {"token": , "content": , "chunk_order_index": , "full_doc_id": , }}
            await self.chunk_entity_relation_graph.upsert_chunk_node(inserting_chunks)
