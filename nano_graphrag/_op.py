...
async def _merge_nodes_then_upsert(
    ...
    new_source_ids = list(set(dp["source_id"] for dp in nodes_data) - set(already_source_ids))
    ...
    await knwoledge_graph_inst.upsert_chunk_edge(
        entity_name,
...
async def _merge_edges_then_upsert(
    ...
    for need_insert_id in [src_id, tgt_id]:
        if not (await knwoledge_graph_inst.has_node(need_insert_id)):
            ...
            source_ids = list(set([dp["source_id"] for dp in edges_data]))
            await knwoledge_graph_inst.upsert_chunk_edge(need_insert_id, source_ids)
    description = await _handle_entity_relation_summary(
    ...
    triple_data = dict(
        ...
        tgt_id=tgt_id,
        description=description,
        source_id=source_id, order=order,weight=weight
    )
    return triple_data
...
async def extract_entities(
    ...
    # all_triples_data = await asyncio.gather(
    #     *[
    ...
    all_triples_data = []
    for k, v in maybe_edges.items():
        result = await _merge_edges_then_upsert(k[0], k[1], v, knwoledge_graph_inst, global_config)
        all_triples_data.append(result)
    ...
    if triple_vdb is not None and all_triples_data and any(all_triples_data):
        triple_data_for_vdb = {
            compute_mdhash_id(dp["src_id"]+dp["tgt_id"], prefix="tri-"): {
                "content": dp["description"],
                "source_e_name": dp["src_id"],
                "target_e_name": dp["tgt_id"],
            }
            for dp in all_triples_data if dp
        }
        await triple_vdb.upsert(triple_data_for_vdb)
...
async def generate_community_report(
    ...
    for level in levels:
        ...
        for c in this_level_community_values:
            report = await _form_single_community_report(c, community_datas)
            this_level_communities_reports.append(report)
...
async def _find_most_related_text_unit_from_entities(
    ...
    chunk_list =[]
    for text_unit in all_text_units[:10]:
        if text_unit is not None:
            if text_unit.get("data",{}) is not None:
                data = text_unit.get("data")
                if data is not None:
                    chunk = data["content"]
        ...
        # chunk_list.append(text_unit["data"]["content"])
    ...
    return all_text_units,chunk_list
...
async def _build_local_query_context(
    ...
    use_text_units, chunk_list = await _find_most_related_text_unit_from_entities(
...
async def local_query(
    ...
    context, chunk_list = await _build_local_query_context(
        query,
    ...
    system_prompt = PROMPTS["local_rag_response"].format(response_type=query_param.response_type, context_data=context)
...
async def _build_my_query_context(
    query,
    knowledge_graph_inst: BaseGraphStorage,
    entities_vdb: BaseVectorStorage,
    triples_vdb: BaseVectorStorage,
    chunks_vdb: BaseVectorStorage,
    text_chunks_db: BaseKVStorage[TextChunkSchema],
    query_param: QueryParam,
):
    # vdb triples query
    triple_results = await triples_vdb.query(query, top_k=query_param.top_k_triples)
    if not len(triple_results):
        return None
    # Bước 1: Tạo dict trống để lưu danh sách điểm cho từng entity
    phrase_scores = {}
    for result in triple_results:
        source = result["source_e_name"]
        target = result["target_e_name"]
        score = result["distance"]

        if source not in phrase_scores:
            phrase_scores[source] = []
        phrase_scores[source].append(score)
        if target not in phrase_scores:
            phrase_scores[target] = []
        phrase_scores[target].append(score)
    # Bước 2: Tính điểm trung bình
    linking_score_map = {}
    for phrase in phrase_scores:
        scores = phrase_scores[phrase]
        num_sources_chunks = await knowledge_graph_inst.get_num_sources_chunks(phrase)
        linking_score_map[phrase] = float(np.mean(scores))/num_sources_chunks
    # có thể thêm lọc top-k node

    # vdb chunks query
    chunk_results = await chunks_vdb.query(query, top_k=query_param.top_k_chunks)
    chunk_scores = {}
    ...
    for k,v in chunk_scores.items():
        chunk_scores[k] *= query_param.chunk_weight

    # combine scores
    combined_scores = {**linking_score_map, **chunk_scores}

    ppr_start = time.time()
    chunk_id_score, entity_id_score = await knowledge_graph_inst.ppr(combined_scores, query_param.damping_factor)
    ppr_end = time.time()
    print(f"PPR took {ppr_end - ppr_start:.2f} seconds")
    chunk_list = []
    #get chunks
    for id_score in chunk_id_score[0:query_param.num_context_chunks]:
        chunk = await text_chunks_db.get_by_id(id_score[0])
        if chunk is not None:
            chunk_list.append(chunk['content'])
    # print(f"chunk_list: {chunk_list}")

    logger.info(
        f"Using {len(chunk_list)} chunks"
    )
    ...
    # text_units_context = "\n".join(context_list)
        text_units_section_list.append([stt, chunk])
    text_units_context = list_of_list_to_csv(text_units_section_list)

    # k-hop path
    # print(f"Entity ID Score: {entity_id_score}")
    path = await knowledge_graph_inst.k_hop_path(entity_id_score, query_param.k_hops, query_param.k_paths)
    # print (f"Path: {path}")
    context= f"""
-----Dữ liệu-----
{text_units_context}
...
{path}
"""
    return context, text_units_context, path, chunk_list

async def my_query_context(
    query,
    knowledge_graph_inst: BaseGraphStorage,
    entities_vdb: BaseVectorStorage,
    triples_vdb: BaseVectorStorage,
    ...
    use_model_func = global_config["best_model_func"]
    context, text_units_context, path, chunk_list = await _build_my_query_context(
        query,
        knowledge_graph_inst,
        entities_vdb,
        triples_vdb,
...
async def my_query(
    ...
    prompt = PROMPTS["my_query"].format(
        context_data=context, question=query
    )
    ...
    # with open("/Users/khoi/Documents/DATN/health-care/src/context.txt", "w") as f:
    #     f.write(context)
    return response, context, text_units_context, path, chunk_list
